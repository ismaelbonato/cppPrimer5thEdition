...
public:
    ...
    Employee(const Employee& origin) : name(origin.name), id(++idCounter) {
        std::cout << "Copy constructor called: " << name << ", ID: " << id << std::endl;
    }
    ...
    Employee &operator=(const Employee& origin) {
        ...
        name = origin.name;
        id = ++idCounter;
        std::cout << "Copy assignment operator called: " << name << ", ID: " << id << std::endl;
